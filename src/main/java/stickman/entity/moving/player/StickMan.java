...
public class StickMan extends MovingObject implements Controllable {
    ...
    public void die() {
        this.active = false;
        DataBase.setResidualLife(DataBase.getResidualLife() - 1);  //deduce residual life
        if (this.level != null && DataBase.getResidualLife() > 0) {
            this.level.reset();
        } else if (DataBase.getResidualLife() <= 0) {
            this.level.lose();
        }
    }
